...
router.post('/send', async (req, res) => {
  const { invited_user_id, invited_email, invited_by, message, role } = req.body;
  if ((!invited_user_id && !invited_email) || !invited_by) {
    return res.status(400).json({ error: 'Provide invited_user_id (Keycloak ID) or invited_email, and invited_by' });
  }
  ...
  try {
    ...
    // Resolve or create invitee user
    let resolvedKeycloakId = invited_user_id || null;
    let invitedUserInternalId = null;
    ...
    if (resolvedKeycloakId) {
      ...
      if (invitedUserResult.rows.length === 0) {
        // Try to fetch from Keycloak to verify and then create locally
        try {
          const adminToken = await getKeycloakAdminToken();
          await axios.get(
            `${process.env.KEYCLOAK_SERVER_URL || 'http://localhost:8080'}/admin/realms/${process.env.KEYCLOAK_REALM || 'framesync'}/users/${resolvedKeycloakId}`,
            { headers: { Authorization: `Bearer ${adminToken}` } }
          );
          // If exists in KC, create local user minimally with role 'user'
          const usernameGuess = `user-${resolvedKeycloakId.slice(0, 6)}`;
          const insertRes = await client.query(
            `INSERT INTO users (keycloak_id, username, email, role) VALUES ($1, $2, $3, $4)
             ON CONFLICT (keycloak_id) DO NOTHING RETURNING id`,
            [resolvedKeycloakId, usernameGuess, null, 'user']
          );
          if (insertRes.rows.length) {
            invitedUserInternalId = insertRes.rows[0].id;
          }
        } catch (e) {
          // ignore, will fail below if not resolvable
        }
      } else {
        invitedUserInternalId = invitedUserResult.rows[0].id;
      }
    }

    if (!invitedUserInternalId && invited_email) {
      // Search or create Keycloak user by email, then upsert local
      const adminToken = await getKeycloakAdminToken();
      let kcUserId = null;
      try {
        const searchResp = await axios.get(
          `${process.env.KEYCLOAK_SERVER_URL || 'http://localhost:8080'}/admin/realms/${process.env.KEYCLOAK_REALM || 'framesync'}/users`,
          { params: { email: invited_email }, headers: { Authorization: `Bearer ${adminToken}` } }
        );
        const found = Array.isArray(searchResp.data) ? searchResp.data.find(u => u.email?.toLowerCase() === invited_email.toLowerCase()) : null;
        if (found) kcUserId = found.id;
      } catch (e) {
        // continue to create
      }

      if (!kcUserId) {
        const usernameFromEmail = invited_email.split('@')[0];
        const createResp = await axios.post(
          `${process.env.KEYCLOAK_SERVER_URL || 'http://localhost:8080'}/admin/realms/${process.env.KEYCLOAK_REALM || 'framesync'}/users`,
          { username: usernameFromEmail, email: invited_email, enabled: true, emailVerified: false },
          { headers: { Authorization: `Bearer ${adminToken}`, 'Content-Type': 'application/json' } }
        );
        if (createResp.status === 201) {
          const loc = createResp.headers.location || '';
          kcUserId = loc ? loc.split('/').pop() : null;
        }
      }
      ...
      if (!kcUserId) {
        await client.query('ROLLBACK');
        client.release();
        return res.status(500).json({ error: 'Failed to resolve or create invited user in Keycloak' });
      }

      resolvedKeycloakId = kcUserId;
      // Upsert local user
      const upsertRes = await client.query(
        `INSERT INTO users (keycloak_id, username, email, role)
         ...
         RETURNING id`,
        [kcUserId, invited_email.split('@')[0], invited_email]
      );
      invitedUserInternalId = upsertRes.rows[0].id;
    }

    if (!invitedUserInternalId) {
      await client.query('ROLLBACK');
      client.release();
...
// Create invite (this will work even if user previously rejected)
    ...
    if (io) {
      const notifyId = resolvedKeycloakId || invited_user_id;
      if (notifyId) {
        io.to(`user_${notifyId}`).emit('org-invite', {
        from: invited_by,
